...
namespace System.IO.Compression
{
    ...
    public class ZipStorer : IDisposable
    {
        ...
        public List<ZipFileEntry> Files = new List<ZipFileEntry>();
        ...
        private ZipFileEntry AddStreamAsync(Compression _method, string _filenameInZip, Stream _source, DateTime _modTime, string _comment = null)
        {
            if (Access == FileAccess.Read)
                throw new InvalidOperationException("Writing is not alowed");
        ...
        /// <returns>True if success, false if not.</returns>
        /// <remarks>Unique compression methods are Store and Deflate</remarks>
        private bool ExtractFileAsync(ZipFileEntry _zfe, Stream _stream)
        {
            if (!_stream.CanWrite)
